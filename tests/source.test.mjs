...
test("ships clear cash-flow comparison semantics and visible optional costs", async () => {
  ...
  assert.doesNotMatch(page, /months with an extra weekly or bi-weekly payment/)
  ...
  assert.match(page, /Include taxes and utilities in cash paid after purchase/)
  assert.doesNotMatch(page, /Plain-language read/)
  assert.doesNotMatch(page, /If you bought at/)
  ...
  assert.match(page, /You paid \{money\(totalMortgagePaymentsThroughExit\)\} to the lender/)
  ...
  assert.doesNotMatch(page, /For a sale-price comparison with selling costs/)
  ...
  assert.match(css, /\.monthly-card \.info-popover[\s\S]*right: 0[\s\S]*left: auto[\s\S]*transform: none/)
  ...
  assert.doesNotMatch(css, /\.plain-language-summary/)
