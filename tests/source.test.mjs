...
test("ships the home decision calculator features and defaults", async () => {
  ...
  assert.match(page, /Home equity from buying/)
  ...
  assert.match(page, /Show down payment/)
  assert.match(page, /results\.selectedPrincipalPaid \+ \(includeDownPayment \? results\.downPayment : 0\)/)
  assert.match(page, /Buying includes all mortgage payments—principal and interest/)
  assert.match(page, /Down payment · up-front equity/)
  assert.match(page, /The down payment is shown separately from buying payments/)
  assert.match(page, /comparison-summary with-down-payment/)
